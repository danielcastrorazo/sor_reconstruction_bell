...
class CanvasImage:
    ...
    def __init__(self, canvas_frame, canvas):
        ...
        self.canvas.bind('<Button-2>', self.__middle_mouse_button)  # remember canvas position
        ...
        self._key_input_l_callback = None
        self._right_click_callback = None
        self._mouse_move_callback = None

    def register_middle_click_callback(self, key_callback):
        self._key_input_l_callback = key_callback

    def register_click_callback(self,  click_callback):
        self._click_callback = click_callback

    def register_right_click_callback(self,  click_callback):
        self._right_click_callback = click_callback

    def register_mouse_movement(self,  mouse_move_callback):
        self._mouse_move_callback = mouse_move_callback

    def __mouse_move(self, event):
        if self._mouse_move_callback is None:
            return
        coords = self._get_click_coordinates(event)

        if coords is not None:
            self._mouse_move_callback(is_positive=True, x=coords[0], y=coords[1])

    def __right_mouse_button(self, event):
        if self._right_click_callback is None:
            return

        coords = self._get_click_coordinates(event)

        if coords is not None:
            self._right_click_callback(is_positive=True, x=coords[0], y=coords[1])

    def __middle_mouse_button(self, event):
        if self._key_input_l_callback is None:
            return

        coords = self._get_click_coordinates(event)

        if coords is not None:
            self._key_input_l_callback(is_positive=True, x=coords[0], y=coords[1])
    ...
    def __keystroke(self, event):
        ...
        if event.state - self.__previous_state == 4:  # means that the Control key is pressed
            ...
            if event.keycode in self.keycodes['d']:  # scroll right, keys 'd' or 'Right'
                ...
                self.__scroll_y('scroll', 1, 'unit', event=event)
            elif event.keycode in self.keycodes['l']:
                if self._key_input_l_callback is None:
                    return
                coords = self._get_click_coordinates(event)
                if coords is not None:
